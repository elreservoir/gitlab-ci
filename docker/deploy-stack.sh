...
response=$(curl -k -s -H "X-Vault-Token: $VAULT_TOKEN" "$VAULT_ADDR/v1/$VAULT_PATH")
...
if echo "$response" | jq -e '.data.data' > /dev/null 2>&1; then
  ...
  echo "Successfully loaded $(echo "$response" | jq -r '.data.data | keys | length') environment variables from Vault for $STACK_NAME"
else
